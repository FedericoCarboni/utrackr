...
// https://www.libtorrent.org/udp_tracker_protocol.html
pub const MAX_PACKET_SIZE: usize = 98;
...
fn make_connection_id(secret: &[u8; 8], current_2min: &[u8], ip: &[u8], port: &[u8]) -> [u8; 8] {
    ...
    log::trace!("connection_id: {:x?}", connection_id);
    connection_id
}
...
fn verify_connection_id(
    ...
    connection_id == make_connection_id(secret, &current_2min.to_be_bytes(), ip, port)
        || connection_id == make_connection_id(secret, &(current_2min - 1).to_be_bytes(), ip, port)
}
...
impl Transaction {
    ...
    fn verify_connection_id(&self) -> bool {
        let current_2min = self.timestamp / 120;
        ...
        verify_connection_id(&self.secret, current_2min, &ip, &port, &self.packet[0..8])
    }
